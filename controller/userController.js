...
exports.resetPassword = async(req, res) =>{
 ...
 //update the user password too the new password to the new pasword
    ...
    if(error instanceof jwt_TokenExpiresdError)
      return res.status(400).json({
    message: "link expird, please request a new link"
    })
    res.status(500).json({
      message:error.message
    })
